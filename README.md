...
> ### 🔬 AutoEvolve: Sky Lab UC Berkeley Research
> 
> This repository includes **AutoEvolve**, a [Sky Computing Lab, UC Berkeley](https://sky.cs.berkeley.edu/) research extension that enhances OpenEvolve with **stagnation-aware paradigm generation** and **adaptive search mechanisms**. AutoEvolve addresses a key challenge in LLM-driven evolution: when evolution stagnates, the system automatically generates breakthrough ideas using structured problem analysis, then adapts its exploration strategy based on what succeeds and fails.
> **Enable AutoEvolve features:** See [configs/README.md](configs/README.
md#-autoevolve-advanced-features) for configuration options.

> **Quick Example:** `./run_circle_packing.sh` — runs circle packing with all AutoEvolve features enabled (see [examples/circle_packing/config.yaml](examples/circle_packing/config.yaml))
>
> **Configuration:** See [configs/README.md](configs/README.md#-autoevolve-advanced-features) for all options.
