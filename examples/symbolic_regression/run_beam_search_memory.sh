...
# Run Beam Search + Memory experiments for all symbolic regression problems
# Uses config_beam_search_memory.yaml with memory enabled
...
strategy_config="./config_beam_search_memory.yaml"
...
echo "Starting all Beam Search + Memory experiments..."
echo "Using config: $strategy_config"
echo "Strategy: Beam Search + Memory (beam_width=4, branch_factor=4, memory enabled)"
echo ""
...
for split_config in "${splits[@]}"; do
    ...
    echo ""
    echo "----------------------------------------------------"
    echo "Processing Split: $split_name (Beam Search + Memory)"
    ...
    for (( i=0; i<count; i++ )); do
        ...
        echo "  Launching $split_name - Problem $i ($initial_program_path) with Beam Search + Memory"
        # Run the experiment in the background with beam search flag and memory config
...
echo ""
echo "All Beam Search + Memory experiment processes have been launched in the background."
echo "Waiting for all background processes to complete..."
wait
echo ""
echo "All Beam Search + Memory experiments have completed."
