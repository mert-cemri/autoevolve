...
# Run Adaptive Exploration/Exploitation + Memory experiments for all symbolic regression problems
...
strategy_config="./config_map_elites_adaptive_memory.yaml"
...
echo "Starting all Adaptive + Memory experiments..."
echo "Using config: $strategy_config"
echo "Strategy: MAP-Elites with Adaptive Exploration + Memory"
...
for split_config in "${splits[@]}"; do
    ...
    echo ""
    echo "----------------------------------------------------"
    echo "Processing Split: $split_name (Adaptive + Memory)"
    ...
    for (( i=0; i<count; i++ )); do
        ...
        echo "  Launching $split_name - Problem $i ($initial_program_path) with Adaptive + Memory"
        # Run the experiment in the background with adaptive + memory config
        # Save to openevolve_output_adaptive_memory
        cmd="python ../../openevolve-run.py \"$initial_program_path\" \"$evaluator_path\" --config \"$strategy_config\" --output \"$problem_dir/openevolve_output_adaptive_memory\" --iterations 200"
        eval $cmd &
    done
...
echo ""
echo "All Adaptive + Memory experiment processes have been launched in the background."
echo "Waiting for all background processes to complete..."
wait
echo ""
echo "All Adaptive + Memory experiments have completed."
