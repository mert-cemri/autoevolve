...
# Run Best-of-N experiments for all symbolic regression problems
# Uses config_best_of_n.yaml with strategy-specific parameters
...
strategy_config="./config_best_of_n.yaml"
...
echo "Starting all Best-of-N experiments..."
echo "Using config: $strategy_config"
echo "Strategy: Best-of-N (n_lineages=4, 200 iterations = 200 evaluations)"
echo ""
...
for split_config in "${splits[@]}"; do
    ...
    echo ""
    echo "----------------------------------------------------"
    echo "Processing Split: $split_name (Best-of-N)"
    ...
    for (( i=0; i<count; i++ )); do
        ...
        echo "  Launching $split_name - Problem $i ($initial_program_path) with Best-of-N"
        # Run the experiment in the background with best-of-n flag and config
...
echo ""
echo "All Best-of-N experiment processes have been launched in the background."
echo "Waiting for all background processes to complete..."
wait
echo ""
echo "All Best-of-N experiments have completed."
