...
Run a quick test with all AutoEvolve features enabled:

```bash
# From the repository root
./run_circle_packing.sh
```

This uses `config.yaml` which has all AutoEvolve advanced features enabled:
- **Paradigm Breakthrough**: Automatically generates breakthrough ideas when evolution stagnates
- **Error Retry**: Retries failed generations/evaluations
- **Adaptive Search**: Dynamically adjusts exploration/exploitation ratio
- **Multi-Island Evolution**: 4 islands with diverse seed programs

Output will be saved to `examples/circle_packing/output/` (gitignored).

### Sample Run Output

See [`sample_output/`](sample_output/) for a real run example:
- **`logs/openevolve_*.log`** - Main evolution log showing island seeding, iterations, improvements
- **`logs/stagnation_monitor.log`** - Stagnation detection and paradigm breakthrough triggers
- **`best/best_program.py`** - Final evolved program
- **`best/best_program_info.json`** - Final score and metadata
