...
class Program:
    ...
    visit_count: int = 0  # Times selected as parent for mutation (for visit-count exploration)
...
class ProgramDatabase:
    ...
    def __init__(self, config: DatabaseConfig):
        ...
        # Track when each island's best program was last improved (for stagnation detection)
        self.island_best_changed_at: List[int] = [0] * config.num_islands
        ...
        # When enabled, dynamically adjusts exploration ratio based on recent improvement rate
        self.use_adaptive_search = getattr(config, "use_adaptive_search", False)
        self.adaptive_window_size = getattr(config, "adaptive_window_size", 20)
        ...
        self.recent_improvements: List[float] = []  # Track improvement (1.0) or no improvement (0.0)
        
        if self.use_adaptive_search:
            logger.info(
                f"Adaptive search enabled: window={self.adaptive_window_size}, "
                f"exploration range=[{self.adaptive_min_exploration}, {self.adaptive_max_exploration}]"
            )

        # Stagnation detection and adaptive exploration parameters
        self.exploitation_temperature: float = getattr(config, "exploitation_temperature", 1.0)
        self.stagnation_threshold: int = getattr(config, "stagnation_threshold", 10)
        self.stagnation_multi_child_count: int = getattr(config, "stagnation_multi_child_count", 3)
        self.sibling_context_limit: int = getattr(config, "sibling_context_limit", 5)
    ...
    def add(
        ...
        # Track best fitness before updating (for adaptive search)
        prev_best_fitness = None
        if self.use_adaptive_search and self.best_program_id and self.best_program_id in self.programs:
            prev_best_fitness = get_fitness_score(
                self.programs[self.best_program_id].metrics, self.config.feature_dimensions
            )

        # Update the absolute best program tracking (after population enforcement)
        self._update_best_program(program)

        # Track improvement for adaptive search
        if self.use_adaptive_search:
            self._track_improvement(program, prev_best_fitness)
    ...
    def sample(self, num_inspirations: Optional[int] = None) -> Tuple[Program, List[Program]]:
        ...
        # Increment visit count when selected as parent
        ...
        logger.debug(f"Sampled parent {parent.id} (visit_count={parent.visit_count}) and {len(inspirations)} inspirations")
    ...
    def sample_from_island(
        ...
        Returns:
            Tuple of (parent_program, inspiration_programs, sampling_mode)
            where sampling_mode is one of "exploration", "exploitation", or "weighted"
        """
        ...
        if self.use_adaptive_search and len(self.recent_improvements) > 0:
            exploration_ratio = self._get_adaptive_exploration_ratio()

            # Calculate exploitation ratio to maintain the configured random ratio
            random_ratio = 1.0 - self.config.exploration_ratio - self.config.exploitation_ratio
            random_ratio = max(0.0, random_ratio)  # Ensure non-negative
            exploitation_ratio = 1.0 - exploration_ratio - random_ratio
            exploitation_ratio = max(0.0, exploitation_ratio)  # Ensure non-negative
        else:
            # Use static ratios from config
        ...
        logger.debug(
            f"Sampled parent {parent.id} (visit_count={parent.visit_count}) and {len(inspirations)} inspirations from island {island_id} "
            f"(mode: {sampling_mode}, rand_val: {rand_val:.3f})"
        )
        return parent, inspirations, sampling_mode
    ...
    def save(self, path: Optional[str] = None, iteration: int = 0) -> None:
        ...
        metadata = {
            ...
            "island_best_changed_at": self.island_best_changed_at,  # For stagnation tracking
    ...
    def load(self, path: str) -> None:
        ...
        if os.path.exists(metadata_path):
            ...
            self.island_best_changed_at = metadata.get(
                "island_best_changed_at", [0] * len(saved_islands)
            )
        ...
        # Ensure island_best_changed_at list has correct length (for stagnation tracking)
        if len(self.island_best_changed_at) != len(self.islands):
            self.island_best_changed_at = [0] * len(self.islands)
    ...
    def _update_island_best_program(self, program: Program, island_idx: int) -> None:
        ...
        if current_island_best_id is None:
            ...
            self.island_best_changed_at[island_idx] = self.last_iteration  # Track when best changed
    ...
    def get_island_stagnation(self, island_idx: int) -> int:
        """
        Get the number of iterations since an island's best program improved.

        This is used for stagnation detection - if stagnation exceeds a threshold,
        we can trigger multi-child generation or other interventions.

        Args:
            island_idx: Island index to check

        Returns:
            Number of iterations since the island's best program was improved
        """
        if island_idx >= len(self.island_best_changed_at):
            return 0
        return self.last_iteration - self.island_best_changed_at[island_idx]

    def is_island_stagnating(self, island_idx: int) -> bool:
        """
        Check if an island is stagnating (best hasn't improved for stagnation_threshold iterations).

        Args:
            island_idx: Island index to check

        Returns:
            True if the island is stagnating
        """
        return self.get_island_stagnation(island_idx) > self.stagnation_threshold

    def get_children(self, parent_id: str, limit: Optional[int] = None) -> List[Program]:
        """
        Get all children of a parent program, sorted by iteration_found (most recent first).

        This is used to provide sibling context to the LLM when mutating a parent,
        so it can learn from what worked and what didn't in previous mutations.

        Args:
            parent_id: ID of the parent program
            limit: Maximum number of children to return (defaults to sibling_context_limit)

        Returns:
            List of child programs, sorted by iteration_found (most recent first)
        """
        if limit is None:
            limit = self.sibling_context_limit

        children = [p for p in self.programs.values() if p.parent_id == parent_id]
        children.sort(key=lambda p: p.iteration_found, reverse=True)
        return children[:limit]

    def _track_improvement(self, program: Program, prev_best_fitness: Optional[float]) -> None:
        """
        Track whether adding a program resulted in improvement (for adaptive search)

        Args:
            program: The newly added program
            prev_best_fitness: The best fitness score before adding this program
        """
        # Calculate the new program's fitness
        new_fitness = get_fitness_score(program.metrics, self.config.feature_dimensions)

        # Determine if this was an improvement
        # An improvement is when the new program becomes the best (or first program)
        is_improvement = 0.0
        if prev_best_fitness is None:
            # First program, count as improvement
            is_improvement = 1.0
        elif self.best_program_id == program.id:
            # This program became the new best, it's an improvement
            is_improvement = 1.0

        # Add to recent improvements
        self.recent_improvements.append(is_improvement)

        # Keep only the last window_size improvements
        if len(self.recent_improvements) > self.adaptive_window_size:
            self.recent_improvements = self.recent_improvements[-self.adaptive_window_size:]

        # Log occasionally for visibility
        if len(self.recent_improvements) % 10 == 0:
            improvement_rate = sum(self.recent_improvements) / len(self.recent_improvements)
            logger.debug(
                f"Adaptive search: window size={len(self.recent_improvements)}, "
                f"improvement_rate={improvement_rate:.3f}"
            )

    def _get_adaptive_exploration_ratio(self) -> float:
        """
        Calculate the adaptive exploration ratio based on recent improvement rate

        Returns:
            Current exploration ratio (between adaptive_min_exploration and adaptive_max_exploration)
        """
        if not self.use_adaptive_search or len(self.recent_improvements) == 0:
            return self.config.exploration_ratio

        # Calculate improvement rate over the window
        improvement_rate = sum(self.recent_improvements) / len(self.recent_improvements)

        # Adaptive formula: more improvements → less exploration (exploit the gains)
        # fewer improvements → more exploration (search for new directions)
        exploration_ratio = (
    ...
    def _sample_parent(self) -> Program:
        ...
        if self.use_adaptive_search and len(self.recent_improvements) > 0:
            ...
            if random.random() < 0.05:  # 5% of samples
                improvement_rate = sum(self.recent_improvements) / len(self.recent_improvements)
                logger.info(
                    f"Adaptive search: improvement_rate={improvement_rate:.2f}, "
                    f"exploration={exploration_ratio:.2f}, exploitation={exploitation_ratio:.2f}"
                )
        else:
            # Use static ratios from config
            exploration_ratio = self.config.exploration_ratio
    ...
    def _sample_exploration_parent(self) -> Program:
        ...
        # Use inverse visit-count weighting for exploration
        # Programs with lower visit_count get higher probability of being selected
        # This encourages trying programs that haven't been explored much
        valid_program_objects = [self.programs[pid] for pid in valid_programs]
        weights = np.array([1.0 / (p.visit_count + 1) for p in valid_program_objects])
        weights = weights / np.sum(weights)  # Normalize

        # Sample according to inverse visit-count weights
        selected_idx = np.random.choice(len(valid_program_objects), p=weights)
        return valid_program_objects[selected_idx]

    def _sample_exploitation_parent(self) -> Program:
        """
        Sample a parent for exploitation using softmax-weighted fitness sampling.

        Uses softmax weighting: P(parent) ∝ exp(fitness / temperature)
        Higher temperature → more uniform distribution (more exploration)
        Lower temperature → more greedy selection (more exploitation)
        """
        if not self.archive:
        ...
        # Select candidate pool (island archive if available, otherwise full archive)
        candidate_ids = archive_programs_in_island if archive_programs_in_island else valid_archive
        candidate_programs = [self.programs[pid] for pid in candidate_ids]

        # Use softmax-weighted sampling based on fitness
        fitness_scores = np.array([
            get_fitness_score(p.metrics, self.config.feature_dimensions)
            for p in candidate_programs
        ])

        # Softmax with temperature (for numerical stability, subtract max)
        temperature = self.exploitation_temperature
        if temperature <= 0:
            temperature = 1.0  # Prevent division by zero

        # Compute softmax weights
        exp_scores = np.exp((fitness_scores - np.max(fitness_scores)) / temperature)
        weights = exp_scores / np.sum(exp_scores)

        # Sample according to softmax weights
        selected_idx = np.random.choice(len(candidate_programs), p=weights)
        return candidate_programs[selected_idx]
    ...
    def _sample_from_archive_for_island(self, island_id: int) -> Program:
        """
        Sample a parent from the archive using softmax-weighted fitness sampling,
        preferring programs from the specified island.
