...
class DatabaseConfig:
    ...
    # When enabled, exploration_ratio is dynamically adjusted based on recent improvement rate
    use_adaptive_search: bool = False
    adaptive_window_size: int = 20  # Look at last N iterations for stability
    ...
    exploitation_temperature: float = 1.0  # Softmax temperature for exploitation sampling (higher = more uniform)
    stagnation_threshold: int = 10  # Iterations without improvement before triggering intervention
    stagnation_multi_child_count: int = 3  # Number of children to generate when stagnation detected
    sibling_context_limit: int = 5  # Maximum number of previous siblings to show in prompt

    # Paradigm-based stagnation detection (improvement rate based)
    # Set enable_paradigm_breakthrough=true to enable automatic breakthrough idea generation when stagnant
    enable_paradigm_breakthrough: bool = False  # OFF by default - set to true to enable
    stagnation_window: int = 5  # Number of iterations to track for improvement rate calculation
    stagnation_improvement_threshold: float = 0.01  # Minimum improvement rate (1% total over the window period)
    stagnation_paradigm_samples: int = 3  # Number of paradigm-guided children to generate when stagnant

    # Error retry settings
    # Set enable_error_retry=true to enable automatic retry on generation/evaluation errors
    enable_error_retry: bool = False  # OFF by default - set to true to enable
    max_error_retries: int = 2  # Maximum retries per iteration when evaluation errors occur (only used if enable_error_retry=true)

    # Paradigm generation settings (for breakthrough idea generation when stagnant)
    paradigm_model: str = "gpt-5-mini"  # Model for generating breakthrough paradigms
    paradigm_api_base: str = "https://api.openai.com/v1"  # API base URL for paradigm generation
    paradigm_api_key: Optional[str] = None  # API key for paradigm generation (defaults to OPENAI_API_KEY env var)
