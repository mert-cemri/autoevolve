...
"""
    ...
    async def run(
        ...
        if should_add_initial:
            ...
            # Generate diverse seed programs for each island
            num_islands = self.config.database.num_islands
            if num_islands > 1:
                logger.info(f"Generating {num_islands} diverse seed programs for islands")
                ...
                # Single island - just add the initial program
        ...
        try:
            self.parallel_controller = ProcessParallelController(
                ...
                output_dir=self.output_dir  # Pass output_dir for correct log location
            )
